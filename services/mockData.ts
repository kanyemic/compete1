import { LeaderboardEntry, QuestionCase } from '../types';
...
export const buildMockLeaderboard = (type: 'rating' | 'streak'): LeaderboardEntry[] =>
  Array.from({ length: 10 }).map((_, index) => {
    const score = type === 'rating'
      ? 2800 - (index * Math.floor(Math.random() * 100 + 50))
      : 50 - (index * Math.floor(Math.random() * 3 + 1));
